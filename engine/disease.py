...
class RespiratoryIllness(Disease):
    ...
    def try_to_infect(self, person, state):
        ...
        if state.stove: # this is a propane stove
            smog_factor = person.coeffs.disease_respiratory_propane_factor
        elif state.improved_stove: # this is an improved wood stove
            smog_factor = person.coeffs.disease_respiratory_stove_factor
        else: # there is no stove improvement at all, so there will be no benefit at all
            smog_factor = 0

        youth_factor = person.coeffs.disease_respiratory_youth_factor
        if person.age >= 5:
            youth_factor = 0 # but that's good (unlike the others)
...
class WaterborneIllness(Disease):
    ...
    def try_to_infect(self, person, state):
        """
        XXX TODO this is MISSING the WATER COMPONENT (w*((HH*Wmin)-Wt))(Wt<(HH*Wmin))
        """

        t_sick = self.t_sick(person, state)
        
        latrine_factor, waterpump_factor = (person.coeffs.disease_waterborne_latrine_factor,
...
class Malaria(Disease):
    ...
    def try_to_infect(self, person, state):
        ...
        bednet_factor = person.coeffs.disease_malaria_bednet_factor * family_bednet_coverage

        # village_infection_proportion === I_t / V_t
        village_infection_proportion = \
            state.village_infected_pop / float(state.village_population)
        ...
        num_village_bednets = village_bednets(num_bednets,
                                              num_family_members,
        ...
        infection_factor = \
            person.coeffs.disease_malaria_sir_factor * village_infection_proportion \
            * infection_rate
