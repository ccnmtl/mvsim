...
def setup_people(state,coeffs,tc):
    ...
    for (name,gender,age,health,education,sick,schooling_state,effort) \
            in zip(state.names,state.genders,state.ages,state.health,
...
def marshall_people(people,state):
    ...
    (state.names,state.genders,state.ages,
     ...
     state.schooling_state) = ([p.name      for p in people],
...
def new_child(tc,coeffs,state):
    ...
    return Person(name=name,gender=gender,age=0,health=health,
                  ...
                  health_t2=health,health_t3=health,coeffs=coeffs,tc=tc)
