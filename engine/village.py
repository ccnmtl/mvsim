class Village:
    ...
    def update_population(self):
       ...
       if self.coeffs.enable_epidemic and self.coeffs.enable_free_bednets and \
          self.state.year == self.coeffs.starting_year + self.coeffs.free_bednet_year + 3:
    ...
    def check_epidemic(self):
       ...
       if self.state.village_infected_pop / float(self.state.village_population) >= 0.3:
    ...
    def precipitation_modifier(self):
        ...
        component = deviation ** self.coeffs.epidemic_precipitation_deviation_exponent
    ...
    def family_bednets(self):
       ...
       return len([elem for elem in self.state.owned_items if elem == "bednet"])
    ...
    def bednet_modifier(self):
       ...
       # @@ XXX TODO this is wrong -- it's currently assuming the village has 
       # the same number of bednets as the family, instead of the same proportion
       return self.coeffs.bednet_infection_modifier \
              * ((self.family_bednets() * 2.0) / self.state.population) 

    def infection_rate(self):
       if not self.coeffs.enable_epidemic or \
          self.state.year < self.coeffs.starting_year + self.coeffs.no_epidemics_before:
          return 0
    ...
    def recovery_rate(self):
       return self.coeffs.recovery_rate1 \
              ...
              + self.coeffs.recovery_rate3 * self.state.clinic * self.state.electricity        
    ...
    def died_of_illness(self):
       self.state.malaria_deaths = self.mortality() * self.state.village_infected_pop
       return self.state.malaria_deaths
       
    def born(self):
       self.state.village_births = self.coeffs.birth_rate * self.state.village_population
       return self.state.village_births
       
    def died(self):
       self.state.village_deaths = self.coeffs.death_rate * self.susceptible_pop()
       return self.state.village_deaths

    def sickened(self):
       # jumpstart the SIR model if the timing is right
       if self.coeffs.enable_epidemic and self.state.season and \
          self.state.year == self.coeffs.starting_year + self.coeffs.no_epidemics_before:
          self.state.village_infected_pop = self.coeffs.initial_infected_pop
       
       sickened = self.infection_rate() * self.susceptible_pop() * self.state.village_infected_pop
       if sickened < 0:
          sickened = 0
    ...
    def calculate_taxes(self, taxes_paid_by_family):
        ...
        assert self.coeffs.avg_family_size, \
            "avg_family_size coefficient must be nonzero! Talk to an administrator."

        # divide by the average family size because we're taxing families, not villagers
        num_families_in_village = self.state.village_population / self.coeffs.avg_family_size
    ...
    def update_subsidy_offers(self):
        ...
        # After the first (n) years, it should be highly likely (about 50%) that the government offer to subsidize a road
        if not self.state.road:
            if self.state.year > (self.coeffs.starting_year + self.coeffs.road_subsidy_year):
                if 'road' not in self.state.subsidy_offers:
                    if rand_n(self.tc,100) < 50.0:
                        #self.message("The Government has offered to subsidize the building of a road." +
                        #             "  Its cost has been reduced on the Village Improvements tab.")
                        self.message("road subsidy")
                        self.state.subsidy_offers.append('road')
                        
        # Before there is a road, there should be no offers to subsidize village improvements
        if not self.state.road:
            return

        # Assuming you have a road, then beginning in the year defined in the coeffs,
        # there is a 5% chance on any given turn that an NGO will
        # offer to subsidize a village improvement.

        if self.state.year < (self.coeffs.starting_year + self.coeffs.other_subsidy_year):
            return
        ...
        if not self.state.clinic and 'clinic' not in self.state.subsidy_offers:
            if rand_n(self.tc,100) < 5:
                #self.message("The Government has offered to subsidize the building of a clinic." +
                #             "  Its cost has been reduced on the Village Improvements tab.")
                self.message("clinic subsidy")
                self.state.subsidy_offers.append('clinic')
                
        if not self.state.irrigation and 'irrigation' not in self.state.subsidy_offers:
            if rand_n(self.tc,100) < 5:
                #self.message("An NGO has offered to subsidize irrigation." +
                #             "  Its cost has been reduced on the Village Improvements tab.")
                self.message("irrigation subsidy")
                self.state.subsidy_offers.append('irrigation')
        if not self.state.sanitation and 'sanitation' not in self.state.subsidy_offers:
            if rand_n(self.tc,100) < 5:
                #self.message("An NGO has offered to subsidize sanitation." +
                #             "  Its cost has been reduced on the Village Improvements tab.")
                self.message("sanitation subsidy")
                self.state.subsidy_offers.append('sanitation')

        if not self.state.water_pump and 'water_pump' not in self.state.subsidy_offers:
            if rand_n(self.tc,100) < 5:
                #self.message("An NGO has offered to subsidize a water pump." +
                #             "  Its cost has been reduced on the Village Improvements tab.")
                self.message('water pump subsidy')
                self.state.subsidy_offers.append('water_pump')
        if not self.state.meals and 'meals' not in self.state.subsidy_offers:
            if rand_n(self.tc,100) < 5:
                #self.message("An NGO has offered to subsidize school meals." +
                #             "  Its cost has been reduced on the Village Improvements tab.")
                self.message('meals subsidy')
                self.state.subsidy_offers.append("meals")
        if not self.state.electricity and 'electricity' not in self.state.subsidy_offers:
            if rand_n(self.tc,100) < 5:
                #self.message("An NGO has offered to subsidize a power grid." +
                #             "  Its cost has been reduced on the Village Improvements tab.")
    ...
    def raw_improvement_price(self,improvement):
        prices = dict()
        for i,price in zip(self.coeffs.available_improvements,self.coeffs.improvement_prices):
            prices[i] = price
    ...
    def calc_new_improvements_value(self):
        return sum([self.check_improvement_price(improvement) for improvement in self.state.improvements if improvement != ""])

    def calc_fish_stock(self):
        assert self.coeffs.avg_family_size != 0
        assert self.coeffs.fish_k != 0

        households = self.state.village_population / self.coeffs.avg_family_size
        total_fish_caught = self.state.amount_fish * households \
                            * ((90.0 + rand_n(self.tc,20)) / 100.0)
        
        return max(self.state.fish_stock + \
                   (self.coeffs.fish_growth_rate * self.state.fish_stock * (1.0 - (self.state.fish_stock / float(self.coeffs.fish_k)))) \
                   - total_fish_caught,
                   0)
    def update_fish_stock(self):
        ...
        if self.state.fish_stock < (self.coeffs.fish_k * self.coeffs.fish_stock_warn_threshold):
    ...
    def calc_wood_stock(self):
        ...
        random = (rand_n(self.tc,100) / 1000.00 - 0.05) * self.state.amount_wood
        total_wood_chopped = self.state.amount_wood \
                             * ((95.0 + rand_n(self.tc,10)) / 100.0) \
                             * (self.state.village_population / self.coeffs.avg_family_size)
        return max(self.state.wood_stock + (self.coeffs.forest_growth_rate
    ...
    def update_wood_stock(self):
        ...
        if self.state.wood_stock < (self.coeffs.wood_k * self.coeffs.wood_stock_warn_threshold):
    ...
    def update_precipitation(self):
        ...
        if self.state.precipitation < self.coeffs.drought_threshold:
            ...
            earliest_allowed_drought_year = self.coeffs.starting_year + self.coeffs.no_droughts_before
