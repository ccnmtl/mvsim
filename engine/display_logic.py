...
def add_extra_gameshow_context(context):
    ...
    #if turn.number == 1 or not state.subsistence_met: 
    ...
    extra_display_vars['maize_count'] = len([x for x in state.crops if x == 'Maize'])
    extra_display_vars['cotton_count'] = len([x for x in state.crops if x == 'Cotton'])

    extra_display_vars['num_bednets'] = len([elem for elem in state.owned_items if elem == "bednet"])
    ...
    extra_display_vars['items_in_market'] = [(item, label, price, adjust_price(price)) 
    ...
    extra_display_vars['sellable_items'] = [(item, label, price,
                                             len([x for x in state.owned_items 
                                                  if x == item])) 
                                            for (item, label, price)
                                            in zip(coeffs.market_items,
                                                   coeffs.market_items_labels,
    ...
    extra_display_vars['available_village_improvements'] = [adjust_village_improvement_price_label(i,l,p) for (i,l,p) in zip(coeffs.available_improvements,
                                                                                                                             coeffs.improvement_labels,                                                                                                                             coeffs.improvement_prices)]
