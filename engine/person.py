...
class Person:
    ...
    def __init__(self,name="",gender="",age=0,health=100,education=0,pregnant=False,sick="",
    ...
    def __str__(self):
        return "<Person name=%s gender=%s age=%s health=%d>" % (self.name,self.gender,self.age,self.health)
    ...
    def update_health(self, cals, clinic, electricity):
        # "Health at time t is a function of your health in the previous period, of your nutrition (the
        # difference between your consumed calories and the minimal calorie requirement), and a boost if
        # there is a clinic, a boost if there is electricity, and a loss if you are sick."

        self.health = in_range(0.0,100.0,self.calculate_health(cals, clinic, electricity))
    ...
    def calculate_health(self, cals, clinic, electricity):
        ...
        return in_range(0,100,self.health \
                         + (self.coeffs.health_nutrition_coeff * (cals - self.coeffs.subsistence)) \
                         + (self.coeffs.health_clinic_coeff * clinic * (100.0 - self.health)) \
                         + (self.coeffs.health_power_coeff * clinic * electricity * (100.0 - self.health)) \
    ...
    def starve(self,allocation):
        t_starve = 100.0 * ((self.coeffs.subsistence - allocation) / float(self.coeffs.subsistence))
    ...
    def dehydrate(self,family_water_needs,amount_water,population):
        ...
        t_dehydrate = 100.0 * ((family_water_needs - amount_water) / population) / water_subsistence
    ...
    def productivity(self):
        ...
        # if you're not a baby, your productivity is a function of your education
    ...
    def check_sick(self,state):
        ...
        elif (rand_n(self.tc,100) / 100.0) > (1.0 - self.coeffs.chance_of_getting_the_flu):
    ...
    def check_effort(self, state):
        ...
        if self.effort > self.maximum_effort():
            ...
            # working too hard
        #    d = self.effort - self.optimal_effort()
        #    e = self.coeffs.effort_too_high_health_loss * d
        #    self.decrement_health(e)
        if self.effort < self.maximum_effort():
